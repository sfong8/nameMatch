...
x = pd.read_csv(fr'{filepath}', skiprows=1, header=None, dtype=str, sep='|',encoding='unicode_escape')
x1=x[[0,26]]
x1.columns = ['COMCODE','DESCRIPTION']
x1.to_csv('comcode_desc.csv',index=None)
...
for file in os.listdir(filepath):
    ...
    x1['MM']=x1['MAF-ACCOUNT-MMCCYY'].apply(lambda x:str(x)[:3])
    x1 = x1[x1['MM'] == '020']
    x1['MMYYY'] = mmyyyy
